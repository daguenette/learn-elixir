defmodule KitchenCalculator do
  ...
  def from_milliliter(volume_pair, unit) when unit == :cup,
    do: {:cup, elem(volume_pair, 1) / 240}

  def from_milliliter(volume_pair, unit) when unit == :fluid_ounce,
    do: {:fluid_ounce, elem(volume_pair, 1) / 30}

  def from_milliliter(volume_pair, unit) when unit == :teaspoon,
    do: {:teaspoon, elem(volume_pair, 1) / 5}

  def from_milliliter(volume_pair, unit) when unit == :tablespoon,
    do: {:tablespoon, elem(volume_pair, 1) / 15}

  def from_milliliter(volume_pair, unit) when unit == :milliliter,
    do: volume_pair

  def convert(volume_pair, unit) when unit == :cup,
    do: from_milliliter(to_milliliter(volume_pair), :cup)

  def convert(volume_pair, unit) when unit == :fluid_ounce,
    do: from_milliliter(to_milliliter(volume_pair), :fluid_ounce)

  def convert(volume_pair, unit) when unit == :teaspoon,
    do: from_milliliter(to_milliliter(volume_pair), :teaspoon)

  def convert(volume_pair, unit) when unit == :tablespoon,
    do: from_milliliter(to_milliliter(volume_pair), :tablespoon)

  def convert(volume_pair, unit) when unit == :milliliter,
    do: from_milliliter(to_milliliter(volume_pair), :milliliter)
end
